...
| 10 | `Truy vết` | Command | Danh sách mã neo, phân tách bằng `,` hoặc `;`. `SOURCE = brd` → `QT-<nn>` (quy tắc) · `TR-<nn>` (trường) · `S-<n><a>` (bước/ngoại lệ) · `SC-###` (chép nguyên văn mã của nguồn, vd `SC-001`) · `DK-<nn>` (tiền điều kiện) · `CTRL-<nn>` (điều khiển) — `DK`/`CTRL` là id chạy định nghĩa trong `qa-run.md`, KHÔNG có trong BRD. `SOURCE = spec` → `FR-###` · `SC-###`. Dùng để dựng sheet "Ma trận truy vết". |
