---
description: Thiết kế/cập nhật domain tổng thể cho MỘT module trong docs/roadmap.md — rút entity/FK/enum/rule chung, ghi docs/domain/<module>.md làm nền cho /speckit.specify từng màn.
---

# Domain design cho 1 module

Trước khi specify từng màn của một module, dựng **domain tổng thể** cho module đó: entity, FK, quan hệ, enum, rule chung. Mục tiêu = specify **tách từng màn** nhưng **không vênh model**, vì mọi màn cùng đọc 1 domain doc. Toàn bộ tiếng Việt.
...
Kỳ vọng: **giá trị cột `Module`** trong `docs/roadmap.md` (vd `khdn/devices`, `khdn/payments`). Roadmap KHÔNG đánh số module — nhận tên module, không nhận số.
- **Khớp chính xác** một module (vd `khdn/devices`) → gom mọi item module đó.
- **Prefix** (vd `system` hoặc `system/*`) → gom mọi module bắt đầu bằng prefix (`system/admins` + `system/groups` + `system/roles`…). Dùng khi nhiều module con chia sẻ model (vd Identity: admins/groups/roles cùng User/Role/Permission) — 1 domain doc cho cả cụm, đúng mục tiêu chống vênh. Tên file lấy theo prefix (`system`).
- Trống → đọc roadmap, **liệt kê các module** kèm số item mỗi module, gợi ý cụm nên gom (prefix chung), hỏi người dùng chọn (AskUserQuestion).
- Không khớp module/prefix nào → liệt kê module có thật, hỏi lại. **Đừng đoán.**
...
Có roadmap: lọc item theo `<arg>` — **khớp chính xác** cột Module, hoặc **prefix** (mọi module bắt đầu bằng `<arg>`, xem User Input). Ghi lại danh sách RM-ID + tên màn + thực thể/CRUD + phụ thuộc. Đây là phạm vi domain. Không item nào khớp → báo và dừng.
...
- Tên file: từ `<module>` (hoặc prefix nếu gom cụm) → lowercase, thay mọi ký tự ngoài `[a-z0-9-]` bằng `-`, gộp `-` liền kề (vd `khdn/devices` → `docs/domain/khdn-devices.md`; prefix `system` → `docs/domain/system.md`). Tạo thư mục `docs/domain/` nếu chưa có.
...
Kết thúc: báo module, số entity, số FK, số câu hỏi mở còn lại, và nhắc:
