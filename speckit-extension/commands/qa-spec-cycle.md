...
10. **Selector bền — E2E định vị bằng `data-testid` TRƯỚC TIÊN.** Đây là mặc định cứng của command (dev DFT gắn sẵn theo agent `frontend-angular`), không phải gợi ý. Thiếu testid trên phần tử cần thao tác → **thêm testid vào source** (Blocker 2, Pha 7), CẤM né bằng CSS class / xpath theo cấu trúc / text hiển thị — đó chính là nguyên nhân "chạy lại là fail" khi DOM hoặc nhãn đổi.
...
- [ ] **Pha 2 — Scan & baseline.** Dò framework/thư mục test hiện có (điền vào qa-context những field còn thiếu), test đã có cho spec này chưa, môi trường sẵn sàng chưa → **thông báo phát hiện**, không hỏi lại cái đã dò được. → chi tiết: `.specify/extensions/dft-speckit/references/qa-context-template.md`
  - **Chốt chiến lược selector ngay ở pha này**: `grep -rhoE 'data-testid="[a-z0-9-]*"|attr\.data-testid' <src FE> | head` (nhánh `attr.data-testid` bắt testid **ghép động** của Angular — thiếu nhánh này thì repo tuân thủ đúng quy ước hàng-động lại bị chẩn là "chưa có testid") — có testid trong source → chiến lược = `data-testid`, ghi vào qa-context. Repo chưa có testid nào và qa-context cũng trống mục *Selector* → **mặc định `data-testid`** (kebab `<màn>-<vùng>-<phần tử>`), ghi lại vào qa-context; KHÔNG hỏi lại, KHÔNG rơi về CSS/text.
  - **Recorder cho tester thủ công**: nếu framework E2E có chế độ record/codegen, cấu hình thuộc tính test-id của nó trỏ đúng `data-testid` (vd `testIdAttribute`) và ghi **lệnh record** vào qa-context — để tester record một lượt là ra kịch bản dùng test id, chạy lại không vỡ. Cấu hình thiếu → recorder sinh selector CSS/text giòn, đúng triệu chứng cần diệt.
  - **Replay bền không chỉ là selector**: kịch bản record phải khai **tiền điều kiện dữ liệu** — chạy lại trên dữ liệu đã reset (mục *Seed* của qa-context) hoặc tham số hoá giá trị duy-nhất-mỗi-lượt. Không làm thì lượt chạy lại của kịch bản "Tạo mới" vỡ vì check trùng (QUCTHT §17) dù selector đúng — vẫn là "chạy lại là fail", chỉ đổi nguyên nhân.
...
- [ ] **Pha 4 — Manual TC → xlsx.** **Sinh case ĐỌC TỪ NGUỒN, CẤM ánh xạ lại bộ case cũ** — nguồn đổi mà chỉ sửa cột `Truy vết`/`Nguồn BRD` = cổng phủ tự chấm chính nó, luôn ra 100% (đã gặp: lọt 3 mệnh đề BRD không case nào phủ). Đúng cách: duyệt TỪNG mệnh đề kiểm được của nguồn (mỗi hàng `TR` · mỗi `QT` · mỗi dòng ngoại lệ · mỗi kịch bản · mỗi ca biên · mỗi `SC`) → hỏi *"case nào phủ?"* — khớp thì tái dùng (giữ nguyên ID), thiếu thì thêm mới. Đối chiếu HAI chiều: mệnh đề không case = thiếu; case không mệnh đề = thừa hoặc nguồn thiếu thông tin.
  ...
  - [ ] **Trạng thái UI từ Đặc tả màn hình**: mỗi `### Màn` → case cho 4 trạng thái (tải/rỗng+tổng số/lỗi mạng/có dữ liệu) · từng cột (hiển thị/định dạng/khi rỗng) · lọc-sắp-tìm (rỗng→empty, reset, đổi filter→trang 1, giữ/mất filter) · ẩn-khóa theo quyền · xác nhận + hệ quả lan truyền của hành động phá hủy. Bám tên field + neo màn trỏ (`FR-###` khi `SOURCE = spec`, `QT/TR/S-` khi `brd`), CẤM suy diễn ngoài NGUỒN. Phân công chống trùng khi `SOURCE = brd`: case trạng thái/lọc/quyền lấy từ `Đặc tả màn hình`; case bấm-được/nhãn/danh sách giá trị lấy từ mục mô tả điều khiển — cùng một điều kiện kiểm xuất hiện ở cả hai → giữ MỘT case. Luật giữ-MỘT-case này áp cho **mọi cặp nguồn**, gồm cả **QUCTHT (Pha 4b) × `Đặc tả màn hình`/mô tả điều khiển**: § quy ước chung phát biểu lại một hành vi đã có case (vd che ký tự ô mật khẩu) → **KHÔNG sinh case thứ hai**; bổ sung chuỗi nguyên văn của § vào `Kết quả mong đợi` của case đã có, ghi `QUCTHT §<n>` vào cột 16 `Ghi chú`, và trỏ chính Case ID đó ở bảng Pha 4b.
  ...
  - [ ] **Thứ tự & gom nhóm — case xếp theo trình tự TESTER CHẠY, không theo thứ tự sinh ra.** Thứ tự **DÒNG** trong sheet là hợp đồng bàn giao; **ID chỉ là khoá, KHÔNG phải trục sắp xếp** (xem luật ID ngay dưới). Ba tầng, đúng thứ tự này:
    - **L1 — Màn hình.** Lấy NGUYÊN VĂN danh sách `### Màn` của mục `Đặc tả màn hình` (`SOURCE = spec`) hoặc danh sách màn của BRD (`SOURCE = brd`), xếp theo **thứ tự người dùng điều hướng** (màn vào đầu → màn kế tiếp). CẤM đẻ nhãn màn không có trong nguồn; CẤM gộp hai màn làm một nhóm; CẤM xé một màn thành nhiều nhóm ngang cấp bằng nhãn **mối quan tâm kỹ thuật** tự nghĩ (`— Xác thực`, `— Chống dò`, `— Validation`) hay bằng **tên tính năng** (`Lưu đăng nhập`) — màn nhiều case thì tách nhóm con dạng `<Màn> — <nhóm item>` theo đúng thứ tự L2 (vd `Đăng nhập — Ô Tài khoản`, `Đăng nhập — Nút Đăng nhập`).
    - **L2 — Item trong màn**, theo thứ tự tester thao tác: mở màn/trạng thái tải → **từng điều khiển theo đúng thứ tự xuất hiện trên màn**, mỗi điều khiển chạy hết vòng của nó (hiển thị/nhãn/giá trị mặc định → nhập/đổi giá trị → validation → biên) → từng nút/liên kết → trạng thái sau hành động → luồng end-to-end của màn. **Mọi case của CÙNG một điều khiển phải nằm liền nhau** — hiển thị và hành vi của một ô/nút KHÔNG được nằm ở hai nhóm cách xa nhau (đã gặp: checkbox `Lưu đăng nhập` hiển thị ở dòng 10–12 còn hành vi ở dòng 52–62, tester phải quay lại màn). Case biên/âm nằm **ngay tại điều khiển sinh ra nó**, KHÔNG dồn thành nhóm "Từ điển dữ liệu" riêng.
    - **L3 — Nhóm xuyên suốt, đặt CUỐI file**, CHỈ cho case thật sự không gắn được màn nào: phân quyền toàn cục · dấu vết hoạt động · phiên & điều hướng. Case gắn được vào một màn cụ thể thì **bắt buộc** nằm ở màn đó — kể cả case sinh từ Pha 4b (xem Pha 4b). Nhóm L3 KHÔNG được đặt tên theo nguồn sinh case (`Quy ước chung`, `Từ điển dữ liệu`).
    - Màn dạng danh sách/CRUD thì L2 của màn đó chạy theo luồng nghiệp vụ `Danh sách → Xem → Tạo mới → Chỉnh sửa → Xoá → Nhập/Xuất`. Đây là **ví dụ cho màn CRUD**, KHÔNG phải thang áp cho mọi feature — feature không CRUD (đăng nhập, cấu hình, dashboard) vẫn xếp theo L1/L2 ở trên, CẤM lấy cớ "không khớp bậc nào" để rơi về thứ tự sinh ra.
    - **Tự kiểm — ghi bảng vào `qa-run.md`**, `| Nhóm | Màn | Dải dòng | số case |`, một dòng mỗi nhóm theo đúng thứ tự trong sheet. Ba điều kiện, sai một là chưa xếp lại: (1) **mọi `### Màn` của nguồn xuất hiện ≥1 lần** ở cột `Màn` — màn vắng mặt = case của màn đó đang nằm nhầm nhóm; (2) các nhóm của cùng một màn **liền nhau**, mọi case của một màn nằm **liền DÒNG** (đối chiếu số dòng, KHÔNG đối chiếu ID); (3) mọi nhóm L3 nằm sau toàn bộ nhóm L1. Không có bảng trong ledger = coi như chưa chạy luật này (đã gặp: 155 case, 10 nhóm trộn 4 trục — màn+mối quan tâm, màn trần, tên tính năng, nguồn sinh — mà tự kiểm cũ "mỗi nhóm một dải ID liền" vẫn báo 10/10 đạt).
  - [ ] **ID**: tái dùng NGUYÊN VĂN ID đã có trong xlsx; case mới cấp số ở CUỐI; bỏ case → để trống số, CẤM dồn số/renumber/đổi PREFIX. **Ngoại lệ duy nhất** — bộ case **chưa từng giao tester** (mọi cột thực thi còn trống, không có `Bug ID`): được cấp lại số một lần theo thứ tự đọc ở luật trên; phải ghi vào `qa-run.md` là đã renumber kèm lý do. Đã giao rồi thì vĩnh viễn cấm. **Thứ tự dòng ≠ thứ tự ID**: `csv_to_xlsx.py` ghi dòng theo đúng thứ tự input và ghép dữ liệu tester theo KHOÁ ID, nên case mới cấp số ở cuối vẫn phải được **đặt đúng nhóm/đúng dòng** theo thang thứ tự ở trên — sheet có ID nhảy số trong một nhóm là **ĐÚNG**, không phải lỗi. CẤM renumber chỉ để ID liền dải trong nhóm (sẽ kích `ContentShiftError` exit 3 khi tester đã chấm).
...
- [ ] **Pha 4b — Đối chiếu LUẬT quy ước chung (QUCTHT).** Nguồn: `.specify/extensions/dft-speckit/references/quy-uoc-chung.md` — luật mặc định toàn công ty, spec thường không nhắc lại; dev build theo nó (`agents/*.md`), QA phải test theo nó.
  ...
  - [ ] Case Pha 4b: `Truy vết` gắn id neo có sẵn trong `N` (CẤM đẻ mã mới) · cột 17 = `QUCTHT §<n>` · không giảm neo `N`. **Cột 3 `Nhóm` = nhóm của MÀN/ĐIỀU KHIỂN mà case kiểm**, theo đúng thang L1/L2 của Pha 4 — case § nói về một điều khiển cụ thể (ô nhập, nút, form của màn X) thì nằm trong nhóm của màn X, chỉ case thật sự không gắn được màn nào mới vào nhóm L3. **CẤM dồn toàn bộ case Pha 4b thành một nhóm `Quy ước chung` ở cuối file** — Pha 4b chạy sau Pha 4 nên ID của chúng nằm cuối, nhưng ID không quyết định vị trí dòng; cột 17 `QUCTHT §<n>` đã là dấu nguồn duy nhất, không cần thêm nhóm riêng để đánh dấu (đã gặp: TC-145..155 gom vào `Quy ước chung` trong khi 146 thuộc màn Đặt mật khẩu, 152 thuộc form đăng nhập, 154 thuộc phiên — tester phải quay lại cả 3 màn ở cuối bộ).
- [ ] **Pha 5 — Author auto test.** Sinh test theo tầng đã chọn ở coverage matrix (không map 1:1 từ manual TC), dùng framework khai báo trong qa-context; comment truy vết FR + TC trong mỗi test; requirement manual-only ghi rõ lý do trong ma trận. → chi tiết: `.specify/extensions/dft-speckit/references/test-generation.md`, `.specify/extensions/dft-speckit/references/coverage-matrix.md`
  - [ ] **E2E — thang ưu tiên locator, BẮT BUỘC theo thứ tự**: (1) `data-testid` (API test-id của framework, vd `getByTestId`); (2) `getByRole` + accessible name — đây là cách **duy nhất** được dùng nhãn để định vị, vì nó neo cả ngữ nghĩa ARIA chứ không chỉ chuỗi hiển thị; (3) **hết** — CẤM CSS class, xpath theo cấu trúc DOM, `getByLabel`/`getByPlaceholder`, và khớp text trần (`getByText`/`:has-text`/`text=`) làm locator (nhãn nguyên văn QUCTHT vẫn dùng để **assert**; định vị bằng nhãn chỉ hợp lệ qua bậc (2) role + accessible name).
  - [ ] **Ca tụt xuống bậc (2) hợp lệ — CHỈ MỘT**: phần tử nằm **ngoài source project** (DOM nội bộ của component thư viện bên thứ ba, dialog native trình duyệt) nên không thể thêm testid. Ghi tên thư viện vào `qa-run.md`. Mọi phần tử **trong source project** mà thiếu testid → Blocker 2, KHÔNG có đường tụt bậc (thiếu testid không phải lý do hợp lệ, vì nó luôn đúng tại thời điểm thiếu). Bước 2b quét **cả `getByRole(`** — viết cả suite bằng role + name cho phần tử trong source thì gate FAIL, không phải đường vòng hợp lệ (trừ project đã khai override role-based hợp lệ trong qa-context, xem `test-generation.md`).
  - [ ] Cổng máy cho luật này là **Bước 2b của Pha 6** (quét locator cấm trong thư mục E2E) — không phải tự giác.
  - [ ] **Hàng trong bảng/danh sách**: định vị theo testid ghép **khoá nghiệp vụ** (`course-list-row-<id>`), **CẤM `.nth(i)` / index** — lọc, sắp xếp hay phân trang đổi là chạy lại sai hàng.
  - [ ] Phần tử cần thao tác mà **chưa có testid** → không tự hạ bậc selector: ghi vào `qa-run.md` danh sách `file:vị trí` + tên testid đề xuất, xử lý theo **Blocker 2 (Pha 7)**.
- [ ] **Pha 6 — Quality gate.** Chạy compile/type-check của project (lệnh lấy từ qa-context); grep xác nhận selector/endpoint được assert thật sự tồn tại trong source; chặn assert tầm thường/rỗng. Pattern selector mặc định là `data-testid="…"` / `getByTestId(…)` khi qa-context không khai khác — **mỗi testid test tham chiếu phải có mặt trong source FE**, `MISSING` (không phải testid dựng động) = ảo giác, sửa trước khi qua pha sau. **Cộng thêm Bước 2b — quét chiều nghịch**: grep locator cấm (`.nth(`/`.first(`/`.last(`, `hasText`, `xpath=`, `text=`, `getByRole(`, `getByText(`, `getByLabel(`, `locator('.class')`, `:has-text(`) trong thư mục E2E; mỗi dòng khớp xếp vào 1 trong 4 nhóm — **không phải locator** (comment / API trùng tên) · **khớp text trong `expect()`** · **phần tử ngoài source project** · còn lại → **gate FAIL** (kể cả CSS/xpath nằm trong `expect()`: định vị giòn để assert vẫn là định vị giòn). Thiếu bước này thì test né testid hoàn toàn sẽ không sinh dòng `MISSING` nào và gate xanh rỗng. → chi tiết: `.specify/extensions/dft-speckit/references/quality-gate.md`
- [ ] **Pha 7 — Readiness (no-defer).** Tự dựng môi trường (services → migrate/seed → start backend/frontend background → cài deps test → poll tới ready) rồi gỡ blocker (auth, selector thiếu, seed). **Thiếu `data-testid`** → Blocker 2: thêm testid vào source đúng quy ước (kebab `<màn>-<vùng>-<phần tử>`, hàng động ghép khoá nghiệp vụ), không đổi behavior/markup khác; sửa **lan nhiều file sản phẩm** → dừng, liệt kê chính xác `file:vị trí` + tên testid, chờ duyệt. Lệnh phá hoại (migrate/seed/reset/khởi tạo có trạng thái) chỉ chạy vào **test target dùng-một-lần** đã khai báo trong qa-context; thiếu khai báo → dừng, hỏi (Recommended trước). **Cổng cứng khi cần người quyết** — escalate đúng phần không tự làm được, nêu rõ thiếu gì + lệnh gợi ý, chờ người dùng xử lý rồi tiếp tục, không bỏ ngang. → chi tiết: `.specify/extensions/dft-speckit/references/environment-bringup.md`, `.specify/extensions/dft-speckit/references/blocker-playbook.md`
...
| 6 — Quality gate | Đường dẫn test vừa sinh + source root | **Fact thô**: đuôi log compile/type-check, danh sách `MISSING` đã xác nhận (không phải dynamic) + danh sách `DYNAMIC?` chưa phân xử, **mọi dòng khớp Bước 2b kèm nhóm 0–3 con đề xuất cho từng dòng**, vị trí assert tầm thường + đường dẫn artifact — **cha ra phán quyết PASS/FAIL**, con không tự tuyên bố PASS |
...
- **Thứ tự & gom nhóm đạt**: `qa-run.md` có bảng `| Nhóm | Màn | Dải dòng | số case |` theo đúng thứ tự sheet; mọi `### Màn` của nguồn xuất hiện ≥1 lần ở cột `Màn`; các case của cùng một màn nằm **liền dòng**; mọi nhóm xuyên suốt (L3) nằm sau toàn bộ nhóm màn. Thiếu bảng = chưa đạt.
...
- **E2E**: Pha 6 đã chạy **cả hai chiều** (Bước 2 `MISSING` + Bước 2b locator cấm) và xanh — mọi locator phân giải về một `data-testid` có thật trong source, trừ ngoại lệ đã xếp nhóm *assert* hoặc *ngoài source project* kèm tên thư viện; không hàng bảng nào định vị bằng index; testid còn thiếu đã được thêm hoặc đã escalate tường minh.
