...
- [ ] **Pha 0 — Intake.** Xác định file spec (từ `$ARGUMENTS` hoặc hỏi), rút feature-id + PREFIX (vd `DEV`) dùng cho ID testcase. **Trích nguyên văn danh sách id FR/AC từ chính file spec** (không từ trí nhớ), đếm `N`, ghi cả danh sách + `N` vào `qa-run.md` — đây là mỏ neo đối chiếu của Pha 3/9/Done-when. Tạo `qa-run.md` nếu chưa có, hoặc đọc ledger nếu đã có (đã có ID testcase trong xlsx → tái dùng nguyên văn, cấm renumber, xem Pha 4). → chi tiết (format ledger chuẩn + quy tắc resume): `.specify/extensions/dft-speckit/references/traceability.md` §2–§3 — ledger free-form không theo format là phiên sau không resume được.
  - **Định vị BRD nguồn (cho cột 17 `Nguồn BRD`).** Dự án có `docs/roadmap.md` → tìm item ứng với feature (RM-ID trong tên thư mục spec, tiêu đề spec, hoặc hỏi), đọc trường **`Nguồn`** của item đó: trỏ `docs/brd/….md[#mục]` → ghi vào ledger làm `BRD_ROOT` của feature. `Nguồn = N/A` / trỏ code / không có roadmap / không có `docs/brd/` → ghi `BRD_ROOT = N/A`, cột 17 mọi case điền `N/A`. **CẤM đoán đường dẫn BRD theo tên file** — chỉ lần theo `Nguồn`.
...
- [ ] **Pha 4 — Manual TC → xlsx.** Mỗi acceptance scenario/rule → 1 testcase. **Nếu spec có cấu trúc một-nhà (phát hiện ở Pha 0), làm giàu thêm — case bổ sung, vẫn gắn vào FR/AC hoặc màn tương ứng trong ma trận, không làm giảm neo `N`:** (a) **Biên từ Từ điển dữ liệu** — mỗi field có `Giới hạn`/`Giá trị hợp lệ` → sinh case tại-biên và ngoài-biên (vd `≤ 255` → 255 pass, 256 fail; `0 ≤ x` → 0 pass, -1 fail; tập giá trị → trong/ngoài tập). Số biên lấy từ Từ điển dữ liệu, KHÔNG bịa. (b) **Trạng thái UI từ Đặc tả màn hình** — mỗi `### Màn` → case cho: 4 trạng thái (đang tải/rỗng+tổng số/lỗi mất mạng-timeout/có dữ liệu), từng cột (hiển thị + định dạng + khi rỗng), lọc/sắp/tìm (gồm lọc rỗng→empty, reset, đổi filter→trang 1, giữ/mất filter), ẩn/khóa hành động theo quyền, xác nhận + hệ quả lan truyền cho hành động phá hủy. Bám tên field / số FR mà màn trỏ, không suy diễn ngoài spec. Output cố định tại `<thư mục spec>/testcases-manual.xlsx` (Pha 8/11 đọc/ghi lại đúng file này qua chạy lại script, script tự merge nên giữ nguyên cột tester đã điền). **`ID` là khóa merge**: case đã tồn tại trong xlsx phải giữ nguyên văn ID cũ; script in `WARNING: ... ID ... sẽ mất` khi lệch → dừng, sửa ID, chạy lại. Cách ưu tiên (ít lỗi hơn): viết `testcases-manual.json` (**17 khóa/case**) rồi convert; CSV 17-cột cũng được chấp nhận. **Cột 17 `Nguồn BRD`** điền vị trí BRD mà case truy về, dựng từ `BRD_ROOT` chốt ở Pha 0 + tiêu đề mục cụ thể (`docs/brd/….md#<mục>`); `BRD_ROOT = N/A` → điền `N/A`; không để trống. Chạy `.specify/extensions/dft-speckit/scripts/csv_to_xlsx.py` để dựng file `.xlsx` 2 sheet (Testcases + Ma trận truy vết); 4 cột thực thi để trống cho tester. Báo số case đã sinh. Script chỉ cần `python3`; lần chạy đầu tự tạo venv + cài `openpyxl` (cần mạng lần đầu đó). **Script thoát ≠ 0 và KHÔNG ghi file trong hai ca, cả hai đều là dữ liệu tester đang bị hỏng**: (a) exit **2** — ID cũ có dữ liệu tester biến mất khỏi input; (b) exit **3** — ID giữ nguyên nhưng **Tiêu đề đổi** trong khi tester đã chấm, tức kết quả sẽ bị gắn sang case khác. Cả hai: đọc lại ID + thứ tự từ chính xlsx, sửa input cho khớp, chạy lại. **CẤM tự thêm `--allow-id-loss` / `--allow-content-shift`** để cho qua — hai cờ đó chỉ người dùng bật. **Cấp ID cho case mới ở CUỐI, không chèn vào giữa rồi đánh số lại; bỏ case thì để trống số đó.** → chi tiết: `.specify/extensions/dft-speckit/references/manual-xlsx-format.md`
- [ ] **Pha 4b — Đối chiếu LUẬT quy ước chung (QUCTHT).** Đọc `.specify/extensions/dft-speckit/references/quy-uoc-chung.md` (bắt buộc, một lần trong phiên) rồi **bổ sung case cho mọi điều khoản áp dụng được với feature này**. Đây là nguồn oracle mà spec thường KHÔNG nhắc lại vì nó là luật mặc định toàn công ty — không đối chiếu thì testcase mù với chính chuẩn mà dev đang build theo (`agents/frontend-angular.md`, `agents/backend-abp.md` đều lấy file này làm LUẬT).
  - **Chuỗi trong `" "` của QUCTHT dùng NGUYÊN VĂN** vào cột `Kết quả mong đợi` — chép, không diễn đạt lại. Đây là chỗ file này khác spec: spec cho biết *có gì*, QUCTHT cho biết *chính xác hiện ra chữ gì*.
  - Áp theo thành phần feature thực sự có (đọc `## Đặc tả màn hình` / Từ điển dữ liệu / FR; không có thành phần nào thì bỏ điều khoản đó, không nặn case): trường nhập → §2 độ dài + §3 luật từng loại + §11 thông báo validation; form tạo/sửa → §8 (dấu `*` đỏ, tiêu đề, nhãn nút, luật disable, hủy khi đã nhập) + §10 toast; nút xóa → §9 (tiêu đề + nội dung + thứ tự nút + disable sau nhấn đầu + thông báo ràng buộc FK); bảng danh sách → §7 (phân trang mặc định 10, tùy chọn `{10,20,50,100}`, tìm kiếm contains không phân biệt hoa thường + debounce 300ms, rỗng → `"Không có dữ liệu!"`, sắp mặc định, xóa dòng cuối trang); tệp → §4; nhập/xuất → §13 (**gồm ca chống công thức Excel đã ghi sẵn**: xuất `=1+1`, `-100`, `+84912345678` → giữ nguyên văn); ngày giờ → §12; trạng thái/màu → §16; trùng dữ liệu → §17; đăng nhập/phiên/rate limit → §18; phân quyền → §19 + §21 (thiếu quyền → **ẩn hẳn, không disable mờ**; enforce phía server); mutation → §21 (audit log đúng 1 entry, đồng bộ màn tham chiếu, chặn double-submit, ESC/click nền không đóng form đang nhập).
  - **Xung đột spec ↔ QUCTHT** (spec/BRD nói khác luật, vd nhãn nút `"Đóng"` trong khi §5 CẤM và bắt dùng `"Hủy"`): **KHÔNG tự chọn bên, KHÔNG sinh case cho điểm đó.** Ghi vào `qa-run.md` mục `Xung đột QUCTHT` (trích nguyên văn cả hai bên + §) rồi **hỏi người dùng** bên nào thắng; chế độ non-interactive → ghi blocker rồi HALT theo luật bên dưới. Đây là phát hiện có giá trị, không phải phiền toái — nó bắt lỗi tài liệu trước khi lỗi thành code.
  - Case sinh ở pha này gắn `Truy vết` vào FR/AC mà nó phục vụ (không đẻ mã mới), `Nguồn BRD` theo Pha 0, và **không làm giảm neo `N`**.
...
- **Pha 4b (xung đột QUCTHT)**: spec/BRD nghịch với `quy-uoc-chung.md` → không tự chọn bên, hỏi người dùng.
...
(**dữ liệu tester Pha 4** — exit 2 hoặc 3, **xung đột QUCTHT Pha 4b**, escalate Pha 7, product-bug Pha 10,
**xác nhận ghi CLAUDE.md/AGENTS.md ở Pha 12**) → **KHÔNG được** tự bỏ qua cổng, tự thêm `--allow-id-loss`
hay `--allow-content-shift`, tự chọn bên trong xung đột QUCTHT, tự ý duyệt fix code sản phẩm, hay ghi
test chưa chạy thành "pass". Thay vào đó: ghi 1 bản
...
- `qa-run.md` log đủ 13 pha (Pha 4b tính trong Pha 4).
- xlsx tồn tại với 2 sheet (Testcases + Ma trận truy vết); 4 cột thực thi để trống; cột auto có kết quả (hoặc "chưa chạy" trung thực, không giả pass); **cột 17 `Nguồn BRD` điền đủ mọi dòng** (đường dẫn BRD hoặc `N/A`, không để trống).
- **Đã đối chiếu QUCTHT (Pha 4b)**: `qa-run.md` liệt kê các § đã áp + § đã bỏ kèm lý do ("feature không có bảng danh sách"), và mục `Xung đột QUCTHT` đã được người dùng phân xử hoặc đang là blocker tường minh.
