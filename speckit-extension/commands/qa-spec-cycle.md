...
- **Pha 4 (mất dữ liệu tester)**: `csv_to_xlsx.py` thoát ≠ 0 vì ID cũ có dữ liệu tester biến mất → **dừng, sửa ID cho khớp, chạy lại**. CẤM thêm `--allow-id-loss` để cho qua; cờ đó chỉ người dùng bật.
...
(**mất dữ liệu tester Pha 4**, **xung đột QUCTHT Pha 4b**, escalate Pha 7, product-bug Pha 10, **xác nhận
ghi CLAUDE.md/AGENTS.md ở Pha 12**) → **KHÔNG được** tự bỏ qua cổng, tự thêm `--allow-id-loss`, tự chọn
bên trong xung đột QUCTHT, tự ý duyệt fix code sản phẩm, hay ghi test chưa chạy thành "pass". Thay vào đó: ghi 1 bản
