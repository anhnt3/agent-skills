#!/usr/bin/env bash
# Release thủ công extension lên GitHub (không cần workflow/Action).
# Build zip -> tạo (hoặc cập nhật) GitHub Release + upload asset.
#
# Dùng:
#   ./release.sh            # đọc version từ extension.yml
#   ./release.sh 1.2.0      # ép version
#
...
pkg="dft-speckit"
version="${1:-$(grep -E '^[[:space:]]*version:' extension.yml | head -1 | sed -E 's/.*version:[[:space:]]*"?([^"#]+)"?.*/\1/' | tr -d '[:space:]')}"
...
notes="Spec-Kit extension. Cài:
\`\`\`bash
specify extension add ${pkg} --from ${url}
\`\`\`"
