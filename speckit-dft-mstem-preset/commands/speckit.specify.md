...
- **Domain doc của module** (nếu có): từ item roadmap lấy giá trị cột `Module`, đọc `docs/domain/<module>.md` (đổi `/`→`-` trong tên file). Đây là **nguồn model chuẩn** (entity/FK/enum/rule chung) — dùng nó, **KHÔNG tự rút lại model từ mock** gây vênh với các màn khác cùng module. Doc ghi entity nào "dùng lại framework" thì theo đó, không đẻ lại. Chưa có doc → đề xuất chạy `/speckit.dft-speckit.domain-design <module>` trước (khuyến nghị, không bắt buộc). Phỏng vấn lòi ra model thiếu/sai so với doc → nhắc cập nhật ngược `docs/domain/<module>.md`.
