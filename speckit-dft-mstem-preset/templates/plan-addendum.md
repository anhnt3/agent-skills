...
- **VI. Bảo mật (cơ chế)**: auth flow qua external IdP + ABP JWT resource server (flow cụ thể do plan quyết); permission ABP định nghĩa per feature; rate-limit login nếu áp dụng; upload validate server-side (whitelist loại+dung lượng, sanitize tên, lưu ngoài webroot): [điền].
