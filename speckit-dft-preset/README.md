...
- **Đọc BRD gốc của chính item**: lần theo trường `Nguồn` của item roadmap tới `docs/brd/…` (đầu ra của `brd-import`), không dò theo tên file. Mọi thứ rút ra phải cite `docs/brd/<đường-dẫn>.md#<mục>`. BRD **ghi rõ + không mâu thuẫn** → duyệt gộp một lượt ở cuối GĐ1 thay vì hỏi lại từng câu; BRD **mơ hồ / lệch với code hoặc domain doc** → vẫn hỏi, nội dung BRD làm option `(Recommended)` kèm căn cứ; BRD **im lặng** → hỏi như thường, không được coi là `N/A`. BRD rút ngắn *số lượt hỏi*, không rút ngắn *phạm vi hỏi*.
...
- **Nhận dạng khuôn BRD + quét xung đột quy ước (GĐ1)**: đo "dấu vân tay" H2 lặp trên cây `docs/brd/` để biết BRD viết theo khuôn nào (hồ sơ thiết kế kiểu RUP-UML, SRS kiểu IEEE 830, hay không khuôn) — mỗi khuôn bỏ trống một tập mục cố định, và những mục đó được **nạp thẳng thành dòng `⏳`** vào sổ thay vì chờ phát hiện muộn. Song song, quét **cơ học** xung đột giữa BRD và bộ quy ước chung của dự án (thuật ngữ bị cấm, chuỗi nguyên văn), gom theo nhóm rồi hỏi phân xử — bắt ở đây vì lệnh QA gặp xung đột sẽ dừng không sinh case, mà lúc đó thường không có người trả lời.
- **Bổ sung BRD theo chuẩn quốc tế sau khi spec xong**: file BRD của item được **chèn thêm** phần còn thiếu để tự đủ sinh testcase, theo bộ ghép **Cockburn *fully dressed* / RUP Use-Case Specification** + **ISO/IEC/IEEE 29148:2018 §5.2.8** (mỗi quy tắc `QT-<nn>` mang `Cách kiểm`) + **ISO/IEC 25010** (NFR theo 9 trục, trigger theo thành phần feature) + **Gherkin** cho tiêu chí chấp nhận. Mục mới: `Từ điển dữ liệu` (`TR-<nn>`: kiểu · độ dài · tập giá trị · duy nhất theo phạm vi · thông báo lỗi), `Đặc tả màn hình` (chép từ spec — nguồn case trạng thái UI khi QA đọc BRD), `Quy ước chung áp dụng`, `Chất lượng phi chức năng`, `Tiêu chí chấp nhận`, `Giả định · Phụ thuộc · Ngoài phạm vi`, `Nhật ký cập nhật`; chèn trong mục gốc: tiền/hậu điều kiện và `### Luồng ngoại lệ` đánh số neo vào bước luồng chính (`S-3a`, `S-6b`). **Quyền hạn CHỈ CHÈN** — mọi mục của BA giữ nguyên vị trí và câu chữ, thiết kế của BA KHÔNG bị gom xuống phụ lục; gắn ID là thêm tiền tố vào dòng có sẵn. Không tạo mục `Yêu cầu chức năng (FR)`: neo truy vết là `S-<n><a>` · `QT-<nn>` · `TR-<nn>`, một tầng FR chỉ diễn đạt lại ba thứ đó bằng chữ khác nên là nguồn lặp lớn nhất (đo trên bản tái-cấu-trúc đời cũ: phụ lục chiếm 69% file, 21% câu phía trên là bản sao). Cổng 3 lớp: **MẤT** (rơi lời BA) · **LẶP** (chép lời BA sang chỗ thứ hai — đơn vị đếm là câu ≥60 byte) · **ĐỦ** (đếm field/kịch bản/ngoại lệ/SC/màn/quy ước/NFR so với `spec.md`). Không qua → khôi phục từ `.specify/brd-backup/`. Mục tiêu: `/speckit.dft-speckit.qa-spec-cycle` sinh testcase **từ `docs/brd/` thay vì `spec.md`**. ⚠️ Sau khi bổ sung, markdown là bản gốc — `.docx` chỉ còn là hạt giống, `brd-import` chạy lại sẽ không merge được.
...
```bash
specify preset add --from https://github.com/anhnt3/agent-skills/releases/download/dft-preset-v0.2.0/dft-preset-0.2.0.zip
```
