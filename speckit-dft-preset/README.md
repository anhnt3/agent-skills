...
- **Ghi ngược BRD sau khi spec xong**: những chỗ BRD mơ hồ mà phỏng vấn vừa làm rõ được gom trong sổ rồi ghi **một lượt** qua bảng duyệt tường minh. Mỗi lần ghi bắt buộc kèm khối `## Nhật ký cập nhật` ở cuối file BRD (ngày, **tên người cập nhật**, loại, nội dung cũ → mới). Dòng **`thay đổi`** (xoá/sửa chữ đang có, hoặc thêm nội dung mâu thuẫn) phải hỏi thêm **BA đã duyệt chưa + tên BA**; **chưa duyệt thì cấm sửa**, dòng đó chuyển sang `## Đề nghị sửa BRD — chờ BA duyệt` trong `interview-notes.md`. Dòng **`bổ sung`** (chỉ thêm, không mâu thuẫn) thì không cần BA. Cấm đổi tiêu đề mục và frontmatter — `brd.manifest.yml` khoá node theo `title`.
...
```bash
specify preset add --from https://github.com/anhnt3/agent-skills/releases/download/dft-preset-v0.0.5/dft-preset-0.0.5.zip
```
