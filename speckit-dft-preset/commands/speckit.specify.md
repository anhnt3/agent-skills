...
Tự tìm trong repo, KHÔNG giả định đường dẫn cố định. Không tìm ra thì **hỏi lại** vị trí, đừng đoán. **GĐ1 chưa ghi/sửa bất kỳ file nào** — mọi mutation (roadmap, file sổ) chỉ xảy ra sau xác nhận cuối GĐ1, để phiên hủy giữa chừng không để lại vết bẩn.
...
1. **Chốt số đếm từ nguồn NGOÀI, trước câu hỏi đầu tiên.** Đầu mỗi giai đoạn, in một dòng neo đếm được rồi mới in bảng. Số đếm phải lấy từ file/artifact thật hoặc từ câu trả lời của người dùng — **cấm ước lượng, cấm đếm từ danh sách do chính bạn nghĩ ra** (tự sinh rồi tự đối chiếu với chính nó thì gate vô nghĩa).
   - GĐ2: `phạm vi có K màn → bảng GĐ2 phải có ≥ K dòng gốc` (K đếm từ router/menu, hoặc do người dùng chốt nếu chưa có màn; `K = 0` hợp lệ → bỏ GĐ2).
...
**Bước B — lưới an toàn, đối chiếu sàn.** Đối chiếu bảng Bước A với sàn dưới: nhánh nào của sàn còn thiếu → thêm dòng `⏳` (Nguồn: `sàn`). Sàn là **mức tối thiểu để bắt thiếu, không phải trần**: mọi nhánh tự sinh ở Bước A giữ nguyên, không được cắt cho khớp sàn.
