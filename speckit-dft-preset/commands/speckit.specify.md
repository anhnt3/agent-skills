...
3. **Quyết định chia hai tầng trọng yếu — hỏi cái đáng hỏi, đề xuất cái còn lại**:
   ...
   - *Đã chốt bởi quy ước* (KHÔNG hỏi, KHÔNG `💡` — đóng bằng đường (e) của Sổ theo dõi §3): dòng mà **bộ ba nguồn quy định** đã ấn định sẵn một **giá trị cụ thể**, và feature không có lý do khác đi. Bộ ba: (i) hiến chương `.specify/memory/constitution.md` · (ii) `CLAUDE.md`/`AGENTS.md` của dự án · (iii) file quy ước chung (path + neo `S` đã định vị ở **GĐ1**). **Điều kiện đủ — cả ba phải THỐNG NHẤT**: tra CẢ BA nguồn cho đúng dòng đó; nguồn nào **có nói** thì phải nói **cùng một giá trị**; nguồn không nhắc tới thì bỏ qua (im lặng ≠ xung đột), nhưng **ít nhất một nguồn phải nói giá trị cụ thể**. **File không tồn tại = im lặng** (không phải "chưa tra"): Ghi chú ghi `— (không có file)`; đường (e) vẫn hợp lệ với một nguồn duy nhất nói giá trị cụ thể. Hai nguồn nói khác nhau → **XUNG ĐỘT → CẤM đóng theo đường này**, phải nâng thành **quyết định trọng yếu** và hỏi người dùng chọn bên (kèm trích cả hai giá trị + nguồn), rồi ghi kết quả vào mục sổ **`Xung đột nguồn quy định — đã phân xử`** (mục RIÊNG của file sổ, §7 — KHÔNG dồn vào `Xung đột BRD chờ BA`, đó là bucket BRD↔quy ước; KHÔNG chạy luật GĐ1, KHÔNG chèn dòng vào bảng `Quy ước chung áp dụng` vì sẽ vỡ gate `= S`). **Khác wording nhưng CÙNG một giá trị đo được** (`20 dòng/trang` vs `20 bản ghi mỗi trang`) KHÔNG phải xung đột — thống nhất, đóng bình thường, Ghi chú ghi giá trị chuẩn hoá; chỉ khác **giá trị** hoặc khác **nguyên văn nhãn/thông báo hiển thị cho người dùng** mới là xung đột. **Chưa tra đủ ba nguồn → không dùng đường (e)**, dòng rơi về `💡 đề xuất` bình thường — nhưng **Ghi chú của dòng `💡` đó BẮT BUỘC mang dấu tra**: một trong **hai** dạng (kèm ba marker như đường (e)): (i) `đã tra 3 nguồn — không nguồn nào ấn định`; HOẶC (ii) `đã tra 3 nguồn — <nguồn> ấn định <giá trị>, đề xuất khác vì <lý do gắn feature>` (ca feature có lý do lệch quy ước — lý do này về sau đi vào cột `Lý do nếu ngoại lệ` của mục 10). Gate nhận CẢ HAI dạng; CẤM dùng dạng (i) khi thực tế có nguồn ấn định. Không tra thì không được `💡`: dòng `💡` không mang dấu tra → **quay lại `⏳`, vi phạm gate §9(a)**. Đây là chốt chống né đường (e) — bỏ tra rồi `💡` toàn bộ là escape hatch, không phải lựa chọn. **NGOẠI LỆ tường minh (hai chỗ, không mở rộng)**: dòng của bảng `Quy ước chung áp dụng` (sàn GĐ3 mục 10) và hai dòng thang `Ưu tiên`/`Cách kiểm` (sàn GĐ3 mục 13) **KHÔNG thuộc phạm vi dấu tra và KHÔNG đếm vào `tổng dòng 💡` của gate** — mục 10 chính là bản kê từng `§` của file quy ước (tra bộ ba trên chúng là vòng tròn, và ghi "không nguồn nào ấn định" là khai man), mục 13 chốt thang phân loại chứ không phải giá trị bị quy ước ấn định; cả hai duyệt gộp theo cơ chế riêng của mục đó (KHÔNG sinh thêm lượt hỏi — thứ yếu không tốn lượt hỏi riêng, xem gạch đầu dòng trên); chỉ hỏi khi phát hiện XUNG ĐỘT.
   - Tầng "đã chốt bởi quy ước" **chỉ áp cho dòng thứ yếu**. Dòng **trọng yếu** (dữ liệu · quyền · luồng nghiệp vụ) dù cả ba nguồn có nói gì vẫn PHẢI phỏng vấn — quy ước chốt cách làm, không chốt nghiệp vụ.
...
- **BRD của chính item** (`docs/brd/…`) — nguồn nghiệp vụ gốc, đọc trước khi phỏng vấn, KHÔNG bắt buộc. Định vị DUY NHẤT qua trường **`Nguồn`** trong khối chi tiết của item roadmap (đường dẫn tương đối gốc repo). CẤM: đoán theo tên file; dùng link ô `ID` của bảng tổng (hệ quy chiếu khác — ghép `docs/` vào sẽ ra đường dẫn không tồn tại).
  ...
  **Nhận dạng KHUÔN của BRD** (trước khi rút nội dung — mỗi khuôn bỏ trống một tập mục cố định, biết khuôn = biết trước phải hỏi gì):
  ...
  - [ ] Đo dấu vân tay: `grep -rh '^## ' <thư-mục-BRD> | sort | uniq -c | sort -rn` (`<thư-mục-BRD>` = thư mục `docs/brd/…` chứa `Nguồn` của item; không có `Nguồn` → `docs/brd`) — toàn cây, chỉ đọc heading (ngoại lệ có chủ ý của luật không-đệ-quy). H2 lặp ≥3 file = khuôn. **Thư mục có <3 file BRD** → tiêu chí "lặp" vô nghĩa, BỎ nó và khớp trực tiếp: đọc `^## ` của chính file BRD mà `Nguồn` của item trỏ tới, khớp hàng nào có ≥3 H2 xuất hiện trong file đó. CẤM để `M = 0` chỉ vì ít file.
  - [ ] In dấu vân tay cho người dùng → tra bảng. Khớp một hàng = ≥3 H2 của hàng đó có mặt; <3 hoặc khớp nhiều hàng → mô tả + hỏi người dùng, CẤM tự xếp. (Tiêu chí này áp cho CẢ hai nhánh ≥3 file và <3 file; khác nhau chỉ ở tập H2 đem khớp — toàn cây hay riêng file của item.)
  ...
  | Không mục nào lặp (và đã áp cả nhánh <3 file ở trên) | **Không theo khuôn** | coi như im lặng toàn bộ — chạy đủ sàn GĐ2/GĐ3; `M = 0`, **ghi lý do cụ thể vào sổ** (`M = 0` vì không H2 nào khớp ≥3 mục của bất kỳ khuôn nào — CẤM `M = 0` trống) |
  ...
  - [ ] Mục khuôn trùng mục sàn → VẪN TẠO DÒNG RIÊNG mang `Nguồn = khuôn BRD` (đếm vào `M`), trạng thái `✅ (đã phủ tại sàn GĐ<x> mục <n>)` **gán về sau ở GĐ2/GĐ3**, khi dòng sàn tương ứng đã `✅` (lúc nạp ở GĐ1 mọi dòng khuôn đều `⏳`); phần khuôn nêu mà sàn chưa trả lời → thêm `⏳`. CẤM `N/A vì đã có ở dòng sàn`, CẤM chỉ ghi chú vào dòng sàn thay vì tạo dòng (M sẽ không bao giờ đạt).

  **Quét xung đột BRD ↔ quy ước chung** (dùng path + neo `S`/`T` đã định vị ở khối **"File quy ước chung — định vị NGAY Ở GĐ1"** — CẤM định vị lại hay hỏi lại ở đây; không có file → bỏ khối này, không nhắc lại). Quét cơ học, không đọc hiểu:
  ...
  - [ ] In bảng **đủ `T` dòng** `| § | Số mục đã quét | Số lần khớp | Điểm khớp (BRD đang ghi → quy ước bắt buộc) |` — ghi cả dòng `0`. Thiếu dòng = **CẤM in Kết GĐ1** (đây là điểm cưỡng chế của neo `T`; GATE GĐ2 sẽ đếm lại từ file scan). CẤM kết luận "không xung đột" khi chưa in đủ `T` dòng.
...
- **Hiến chương** (`.specify/memory/constitution.md`) — đọc toàn bộ, **để dùng ở GĐ4 (đối chiếu)**, KHÔNG dùng làm khung câu hỏi cho GĐ2–GĐ3. Đếm số nguyên tắc → neo `N`: đếm các **heading nguyên tắc** trong section nguyên tắc của hiến chương (`grep -cE '^### ' <hiến chương>` với khuôn hiến chương preset ship; khuôn khác → in danh sách tên nguyên tắc đã đếm để người dùng đối chiếu, CẤM chỉ đưa con số). Đây cũng là mẫu số kiểm cite `§x ≤ N` của đường (e). Không có file này → cảnh báo người dùng nên copy hiến chương vào `.specify/memory/` (preset chỉ swap template, không tự ghi file sống); người dùng vẫn muốn tiếp → **bỏ GĐ4**, GĐ2–GĐ3 chạy đủ như thường.
...
- **File quy ước chung — định vị NGAY Ở GĐ1, vô điều kiện. LÀM TRƯỚC mọi bullet khác của GĐ1** (khối quét xung đột BRD tiêu thụ path + `S`/`T` này; chạy tuần tự tới đó mà chưa có path là do bỏ bước, KHÔNG phải lý do bỏ quét) (không chờ GĐ3 mục 10, không phụ thuộc item có BRD hay không; đường (e) của Sổ theo dõi §3 dùng nó ngay từ GĐ2): (1) `.specify/extensions/dft-speckit/references/quy-uoc-chung.md` nếu extension đã cài; (2) không có mới hỏi người dùng — **gộp vào lượt hỏi chung GĐ1**, không tốn lượt riêng; khi đó path chỉ có sau lượt hỏi gộp → khối quét xung đột chạy **ngay sau khi lượt gộp trả về path, trước Kết GĐ1** (không phải lý do bỏ quét). Chốt path → đếm `S = T` bằng `grep -cE '^## [0-9]+\. '`, đưa cả path lẫn `S` vào BẢNG NEO đầu GĐ2. Không có file → `S = T = 0`, ghi rõ, đường (e) chỉ còn hai nguồn.
...
1. **Chốt số đếm từ nguồn NGOÀI, trước câu hỏi đầu tiên.** Đầu mỗi giai đoạn, in một dòng neo đếm được rồi mới in bảng. Số đếm phải lấy từ file/artifact thật hoặc từ câu trả lời của người dùng — **cấm ước lượng, cấm đếm từ danh sách do chính bạn nghĩ ra** (tự sinh rồi tự đối chiếu với chính nó thì gate vô nghĩa).
   ...
   - GĐ3: `Bước A có P dòng; bổ sung Q dòng từ sàn → bảng cuối có ≥ P+Q dòng` (sổ SỐNG có thể append thêm, xem §5). P là danh sách tự sinh (không artifact nào liệt kê nghiệp vụ nền) nên KHÔNG mạnh như neo K/N — bù bằng cột `Nguồn` bắt buộc từng dòng (xem GĐ3).
   ...
   - **In một BẢNG NEO duy nhất ở đầu mỗi giai đoạn** thay vì rải neo: `| Ký hiệu | Nguồn đếm | Giá trị | Đã có trong bảng |`. Tám ký hiệu, gộp thành bảy dòng (`P`/`Q` chung một dòng): `K · N · M · T · S · J · P/Q`. `T` và `S` CÙNG GIÁ TRỊ (cùng lệnh đếm trên file quy ước) nhưng kiểm HAI bảng khác nhau — `T`: bảng quét xung đột GĐ1, `S`: bảng mục 10 GĐ3; đừng nhầm `S` (neo đếm) với tiền tố bước `S-<n>`. Ký hiệu không áp dụng → ghi `0` kèm lý do, CẤM bỏ dòng.
2. **Bảng**: `| # | Nhánh | Tầng | Nguồn | Trạng thái | Ghi chú |` — **cột `Nguồn` bắt buộc cho CẢ GĐ2 và GĐ3** (neo `M` đếm theo cột này; GĐ2 thiếu cột thì không đếm được). Tầng ∈ `trọng yếu` · `thứ yếu` (theo Bất biến #3). Nguồn ∈ `khuôn BRD` · `BRD <cite>` · `domain doc` · `liên hệ GĐ1` · `nợ kỹ thuật` · `GĐ2` · `sàn` · `mock` · `phán đoán BA`. Trạng thái ∈ `⏳ chờ` · `💡 đề xuất` (chỉ cho nhánh thứ yếu; Ghi chú chứa giá trị đề xuất + căn cứ) · `✅ đã chốt` · `N/A vì <lý do cụ thể gắn feature>`. (GĐ4 dùng bộ trạng thái riêng, xem GĐ4.)
3. **Điều kiện đánh `✅`**: (a) nhánh đã có ≥1 câu AskUserQuestion nhận được phản hồi; HOẶC (b) suy trực tiếp từ fact tra cứu (ghi rõ nguồn) — gồm dòng lấy từ BRD **đã** qua bảng `BRD đã chốt sẵn` ở GĐ1, Ghi chú phải mang cite `docs/brd/…md#<mục>`; HOẶC (c) dòng `💡` đã qua duyệt gộp ở recap cuối giai đoạn (§8); HOẶC (d) dòng khuôn BRD trùng một mục sàn — ghi `✅ (đã phủ tại sàn GĐ<x> mục <n>)`, **chỉ hợp lệ khi dòng sàn được trỏ tới đã `✅`**, và phải rà phần khuôn nêu mà dòng sàn chưa trả lời. HOẶC (e) **đóng theo quy ước** — dòng **thứ yếu** thỏa đủ điều kiện bộ-ba-thống-nhất của Bất biến #3: ghi trạng thái trung gian **`✅⏸ (theo quy ước — chờ phủ quyết)`**, Ghi chú **bắt buộc** mang **giá trị cụ thể** + **cite cả ba nguồn đã tra**, dạng `<giá trị> — hiến chương §<x> · CLAUDE.md#<mục> · <path quy ước>#<§n>` (nguồn không nhắc → `— (không nhắc)`; nguồn không có file → `— (không có file)`; CẤM bỏ trắng để giấu việc chưa tra). **Kiểm cite bằng lệnh, một lần cho mỗi giai đoạn**: `grep -nE '^#{2,3} ' <hiến chương> <CLAUDE.md hoặc AGENTS.md> <file quy ước>` — MỌI cite phải khớp một heading có trong output đó, hiểu chính xác theo MỘT hệ quy chiếu duy nhất cho mỗi file: `§n` của **file quy ước** = mục đánh số `## n.` (đúng regex đếm `S`: `^## [0-9]+\. `) — KHÔNG phải heading thứ n của output, vì `## Mục lục` cũng là heading; `§x` của **hiến chương** = **nguyên tắc thứ x** (đếm đúng cách đã dùng cho neo `N`, không đếm các heading phi-nguyên-tắc kiểu `## Governance`); `<file>#<mục>` (CLAUDE.md/AGENTS.md) = **tên heading nguyên văn**, không đánh số. Dự án dùng `AGENTS.md` thay `CLAUDE.md` → cite ghi `AGENTS.md#<mục>`, giữ nguyên ba vị trí marker. **Cite hiến chương KHÔNG kiểm bằng output grep** (nguyên tắc có thể trình bày bằng danh sách đánh số/bold, không phải heading) — tiêu chí là chỉ số: phải là `§x` với `1 ≤ x ≤ N`, cite file quy ước phải là `§n` với `n ≤ S` và `## n.` phải tồn tại thật trong file. Cite không khớp output, hoặc kiểu `theo chuẩn chung` không nói chuẩn nào → **không hợp lệ, dòng quay lại `⏳`**. `✅⏸` hợp lệ tại GATE §9(b) nhưng **CẤM chuyển giai đoạn khi còn `✅⏸`** — chỉ thành `✅` sau khi bảng (b2) của recap §8 được người dùng phản hồi. Cấm tự đánh `✅` ngoài năm đường này — đọc thấy trong BRD nhưng chưa qua bảng duyệt GĐ1 thì **chưa** được `✅`.
...
7. **File sổ — hồ sơ bền ngoài hội thoại**: tạo `.specify/interviews/<slug>.md` (slug kebab-case không dấu sinh từ `$ARGUMENTS`) ngay sau xác nhận GĐ1. Cập nhật (ghi đè toàn bộ) **sau mỗi lượt AskUserQuestion có quyết định được chốt**, vào cuối mỗi giai đoạn, và ngay trước mỗi lần xin xác nhận — phiên dài chắc chắn bị tóm tắt context, file chỉ cập nhật cuối giai đoạn là hổng đúng đoạn dễ trôi nhất. Nội dung: neo đếm, bảng sổ đầy đủ, recap các quyết định đã chốt, ràng buộc `→ plan` (GĐ4), mục `Nợ chờ ghi`, mục `Xung đột BRD chờ BA`, mục `Xung đột nguồn quy định — đã phân xử` (xung đột giữa hiến chương ↔ `CLAUDE.md` ↔ file quy ước, theo Bất biến #3 — mục RIÊNG, không trộn với bucket BRD). Đây là **nguồn sự thật**: phiên dài bị tóm tắt context / bảng trôi mất → đọc lại file này, CẤM dựng lại bảng từ trí nhớ.
8. **Recap cuối giai đoạn — thứ người dùng duyệt là NỘI DUNG**: đạt GATE (§9) rồi mới in, theo thứ tự: (a) recap nội dung các quyết định đã chốt, gom theo màn/nhóm nhánh; (b) **bảng duyệt gộp** các dòng `💡` (nhánh + giá trị đề xuất + căn cứ) — người dùng chỉnh dòng nào thì chốt theo bản chỉnh, xác nhận đồng nghĩa duyệt các dòng còn lại; (b2) **bảng `Đóng theo quy ước`** — mọi dòng đã `✅` qua đường (e) của §3: `| Nhánh | Giá trị đã áp | Hiến chương | CLAUDE.md | Quy ước chung |`. Đây là chỗ người dùng **phủ quyết** được: dòng họ nêu ra trong tin nhắn xác nhận thì mở lại thành câu hỏi trọng yếu; dòng **không được nhắc tới trong tin nhắn xác nhận** = duyệt. **KHÔNG có tin nhắn xác nhận = CHƯA duyệt** (Bất biến #6) — CẤM suy "người dùng im lặng nên đồng ý". CẤM gộp bảng này vào bảng `💡` hoặc nén thành một câu văn xuôi. Không có dòng (e) nào → vẫn in bảng với một dòng `0 dòng đóng theo quy ước` (đối xứng luật `0/0` ở §9). Người dùng phủ quyết dòng nào → dòng đó về `⏳`, hỏi như quyết định trọng yếu, chốt xong **chạy lại GATE §9 từ đầu** rồi mới in lại recap; (c) bảng sổ cuối + dòng đối chiếu số đếm. Sau phản hồi của người dùng: chuyển các `💡` thành `✅` (theo giá trị đã chỉnh nếu có) **và mọi `✅⏸` không bị phủ quyết thành `✅`**, cập nhật file sổ, rồi mới chuyển giai đoạn.
9. **GATE (không cảm tính) — đủ cả hai mới được in recap xin xác nhận**:
   ...
   S (bảng mục `Quy ước chung áp dụng`): <đếm>/<S>   ·   J: <đếm>/<J>   ·   P: <đếm>/<P> · Q: <đếm>/<Q>
   Đóng theo quy ước (đường (e), §3): <m> **trong số** <n> dòng `✅⏸` có ĐỦ BA vị trí cite/marker VÀ ≥1 trong đó trỏ GIÁ TRỊ CỤ THỂ / <n> dòng `✅⏸` (chỉ đếm trong tập `✅⏸`; dòng `💡` mang dấu tra dạng (ii) KHÔNG đếm vào `m`) — **m phải = n**; mọi cite đã kiểm hợp lệ **theo đúng hệ quy chiếu từng file ở §3(e)** (quy ước: khớp heading `## n.` có thật · hiến chương: `1 ≤ x ≤ N` · `CLAUDE.md`/`AGENTS.md`: khớp tên heading nguyên văn): <có/không>
   Dấu tra bộ ba: <đếm dòng `💡` có dấu tra>/<tổng dòng `💡` — KHÔNG đếm dòng mục 10 & mục 13 của sàn GĐ3, ngoại lệ tường minh ở Bất biến #3> — **phải bằng nhau**; thiếu → dòng thiếu quay lại `⏳`, vi phạm gate
   Tỉ lệ dùng đường (e): <n> dòng `✅⏸` / <tổng dòng thứ yếu của giai đoạn — ĐẾM cả dòng thứ yếu đã `✅` qua đường (b)/(c)/(d); KHÔNG đếm dòng `N/A`, dòng trọng yếu, và dòng mục 10 & mục 13 của sàn GĐ3> — tỉ lệ > 2/3 → in danh sách n dòng kèm cite để người dùng soi, đây là dấu hiệu lạm dụng
   ```
   Neo nào `0` thì vẫn in `0/0` kèm lý do — bỏ dòng = vi phạm gate. **Giai đoạn hiệu lực của từng neo** (ngoài giai đoạn hiệu lực thì in `—` kèm tên giai đoạn sẽ chốt, KHÔNG tính là thiếu dòng, cũng KHÔNG được bỏ dòng): `K` → GĐ2 · `M` → chung cuộc tại GATE GĐ3 (vì đối chiếu GĐ2 **+** GĐ3 cộng lại; tại GATE GĐ2 in `<đếm GĐ2>/— (chốt ở GĐ3)`) · `T` → **cưỡng chế ngay cuối khối quét GĐ1**: chưa in đủ `T` dòng thì CẤM in Kết GĐ1; và in lại một lần ở GATE GĐ2 dạng `T: <số dòng bảng quét đếm lại từ file scan>/<T>` (bảng quét đã ghi ra file scan nên đếm lại được) — KHÔNG in `—` cho `T` ở GĐ2 · `S` → GĐ3 (GĐ2 in `—`) · `J`, `P`, `Q` → GĐ3 · `N` → GĐ4. CẤM tự chế ký hiệu miễn trừ khác. GĐ4 in thêm dòng `N: <đếm>/<N>` (số nguyên tắc hiến chương — chỉ có giá trị ở GĐ4, giai đoạn khác ghi `—`). Thiếu dòng so với danh sách nguồn → bổ sung `⏳` rồi hỏi, cũng là vi phạm gate. **`m < n` ở dòng `Đóng theo quy ước` → mọi dòng thiếu cite-giá-trị-cụ-thể quay lại `⏳`, cũng là vi phạm gate** — CẤM đi tiếp bằng cách sửa con số.
   (b) **không còn `⏳`**: mọi dòng phải `✅`, `✅⏸`, `💡` hoặc `N/A` — **trừ GĐ4**, dùng bộ disposition riêng của GĐ4 (`✅ đạt` · `⚠` · `→ plan`, CẤM `N/A`). (`✅⏸` qua được GATE nhưng chặn chuyển giai đoạn — xem §3 đường (e) và §8 (b2).)
...
**Phân tầng trong sàn** (Bất biến #3): thường thì (1), (3) và (5) là trọng yếu — hỏi từng câu; chi tiết của (2)/(4) — sort/lọc mặc định, nội dung empty-state, wording lỗi — thường thứ yếu → `💡 đề xuất`, duyệt gộp cuối GĐ2. **(1), (3), (5) LUÔN trọng yếu — CẤM hạ xuống thứ yếu** dù quy ước/CLAUDE.md/hiến chương có nói gì (đó là luồng · quyền · dữ liệu theo đúng định nghĩa Bất biến #3). "Không máy móc" chỉ đi **một chiều: NÂNG** (2)/(4) lên trọng yếu khi feature đòi — vd empty-state của một màn phê duyệt tài chính. CẤM chiều ngược lại. **Trước khi hỏi hoặc `💡` một dòng thứ yếu, tra bộ ba nguồn (hiến chương · `CLAUDE.md`/`AGENTS.md` · file quy ước chung)**: cả ba thống nhất một giá trị cụ thể → đóng bằng đường (e) của Sổ theo dõi §3, không tốn lượt hỏi; có xung đột giữa các nguồn → nâng thành trọng yếu và hỏi. Dòng (1)/(3)/(5) vẫn hỏi bất kể quy ước nói gì — đường (e) KHÔNG áp cho chúng.

**Nếu dự án có tầng mock/prototype** (theo `CLAUDE.md`/khảo sát): thêm cho mỗi màn một dòng con — function/button/action/label/text nào đang chạy trên dữ liệu giả và cần nối backend thật. Đây là câu hỏi *wiring*, hỏi **sau** năm mục nghiệp vụ trên, không thay thế chúng. Dòng này ghi `Nguồn = mock` (giá trị hợp lệ của enum cột `Nguồn`, §2) — KHÔNG mượn `sàn`, vì nó không thuộc mục sàn (1)–(5) nào.
...
10. **Quy ước chung áp dụng** *(→ mục `Quy ước chung áp dụng` của BRD — số hiệu H2 gán lúc chèn)* — feature áp § nào, ngoại lệ § nào + lý do:
    - **Dùng LẠI path + neo `S` đã chốt ở GĐ1** — CẤM định vị lại, CẤM hỏi lại. Preset và extension PHẢI trỏ cùng một file quy ước (cùng một path duy nhất cho cả hai gói); path GĐ1 khác path extension đang dùng → HALT, hỏi người dùng chọn một. **Ghi path đã dùng vào dòng đầu mục `Quy ước chung áp dụng` của BRD**.
    ...
    - Phân tầng: mặc định cả `S` dòng = `Áp`, ghi `💡`, duyệt gộp recap GĐ3 — **các dòng này KHÔNG cần dấu tra bộ ba và KHÔNG đếm vào gate `Dấu tra bộ ba` (ngoại lệ tường minh ở Bất biến #3)**. § có trong bảng xung đột GĐ1 **đã được hỏi và chốt ở GĐ1** — chỉ ghi kết luận vào ô của dòng đó, CẤM hỏi lại. Chỉ phát sinh hỏi trọng yếu mới khi feature có lý do ngoại lệ cụ thể chưa từng hỏi.
    - Mỗi nhóm xung đột GĐ1 đã chốt → ghi kết luận (theo quy ước / ngoại lệ + lý do) **vào ô của đúng dòng `§` liên quan**, KHÔNG thêm dòng mới (thêm dòng là vỡ gate `= S`).
...
12. **Chất lượng phi chức năng** *(→ mục `Chất lượng phi chức năng` của BRD — số hiệu H2 gán lúc chèn)* — ISO/IEC 25010 làm **lăng kính rà soát, KHÔNG phải 9 ô bắt buộc điền**:
    ...
    |---|---|
    | Functional Suitability | *(phủ bởi FR — đánh `đã phủ`, không hỏi riêng; `đã phủ` là trạng thái HỢP LỆ riêng của bảng 9 trục, ngoài tập `Bắn`/`Bỏ`)* |
    ...
    - Không bắn → `Bỏ` + lý do gắn feature. CẤM `Bỏ` trống/chung chung. Thiếu dòng nào trong 9 = vi phạm gate. Cuối bảng in `Bỏ vì không có mặc định: <n>/9` — **`n ≥ 5` → BẮT BUỘC in, cho từng dòng `Bỏ`, danh sách file đã tra kèm path** (hiến chương · file quy ước · tài liệu NFR); không in được = chưa tra thật = vi phạm gate.
...
- **Ghi nợ roadmap một lượt**: các bullet trong `Nợ chờ ghi` → append vào mục `Nợ phát sinh` của item roadmap tương ứng trong `docs/roadmap.md` (nếu dự án có roadmap); báo lại cho người dùng danh sách nợ đã ghi.
  ...
  - Chạy lại `brd_roadmap.py verify` **sau khi** ghi nợ (chỉ khi extension `dft-speckit` đã cài — không có thì bỏ, không phải lỗi); exit ≠ 0 → sửa ngay, đừng để lại roadmap gãy.
...
> (đo thật trên bản tái-cấu-trúc đời cũ: phụ lục 69% file, 21% câu phía trên là bản sao). Chèn-thêm đưa lặp-nguyên-văn về 0 theo cấu trúc; lặp-diễn-đạt-lại tự giữ bằng **luật gắn ID #4** (luật một-nhà, xem khối "Bốn luật gắn ID" bên dưới — KHÔNG phải Bất biến #4).
...
- Dòng `QT-### có Cách kiểm` và neo `S` (số mục file quy ước) phải đếm bằng ĐÚNG lệnh sau — đếm theo dòng là sai, cả hai đối tượng đều **trải nhiều dòng**:
  ...
  ```
- **Bốn hàng còn lại cũng đếm bằng lệnh** (chạy trên CẢ HAI vế, đối chiếu **danh sách ID** chứ không chỉ số lượng):

  ```bash
  grep -oE 'SC-[0-9]{3}' <file> | sort -u                    # số SC-###
  grep -cE '^### Màn' <file>                                 # số khối `### Màn` (thô)
  grep -ciE '^\s*([-*]|[0-9]+\.)?\s*\*{0,2}(Given|Khi|Cho)\b' <file>  # số kịch bản G/W/T (khớp cả dạng `1. **Given**` của spec-template) — đếm DÒNG keyword, không phải số kịch bản; phép `=` vẫn đúng vì hai vế chép nguyên văn. CẤM 'sửa lệnh cho ra số kịch bản'. Lệch → **trước khi sửa tài liệu, kiểm xem dòng khớp có nằm NGOÀI khối kịch bản không** (FR/Edge Case viết `- Khi …` sẽ khớp nhầm) — loại các dòng ngoài khối rồi so lại
  grep -oE '^\| *`?TR-[0-9]+' <file> | sort -u                # số field/hàng TR-
  ```
  Riêng `### Màn`: số thô ở trên là **trần**; khối thiếu dòng trạng thái hoặc dòng quyền KHÔNG được tính — in danh sách tên màn của cả hai vế rồi loại thủ công từng khối thiếu, ghi rõ khối nào bị loại vì lý do gì.
- Hàng `hộp cảnh báo nhãn lệch`: số hàng = dòng tổng `Điểm lệch còn lại sau phân xử` của bảng scan GĐ1 — trích nguyên văn dòng tổng đó ra khi đối chiếu — đọc từ `specs/<thư-mục-feature>/scan-xung-dot-quy-uoc.md` (file scan GĐ1 đã chuyển sang đây ở bước bàn giao), CẤM nhớ lại con số.
...
- **Cổng lớp 3** (đủ 11 chỉ số, đúng 11 hàng của bảng lớp 3 — thiếu chỉ số = nhật ký không hợp lệ): field <n>/<n> · kịch bản <n>/<n> · ngoại lệ <n>/<n> · SC <n>/<n> · khối `### Màn` <n>/<n> · quy ước <n>/<n> · trục NFR <n>/9 · QT có Cách kiểm <n>/<n> · QT chưa chốt <n>/<trần> · hộp cảnh báo nhãn lệch <n>/<n> · FR có neo <n>/<n>
