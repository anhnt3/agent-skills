...
**Trước GĐ0**: thực hiện mục **"Pre-Execution Checks"** của core bên dưới (đọc `.specify/extensions.yml`, chạy hook `before_constitution` nếu có) rồi mới vào GĐ0 — hook "before" phải chạy trước cuộc phỏng vấn, không phải sau khi mọi thứ đã xong.
...
6. **Không tự phê duyệt.** Có đúng **hai chốt xác nhận bắt buộc**: (a) bảng hợp nhất cuối GĐ4, (b) bản nháp hiến chương đầy đủ cuối GĐ5 — trước khi core ghi file sống. Các giai đoạn đọc-quét (GĐ0–GĐ3) không cần xin xác nhận chuyển giai đoạn. Thứ đưa ra xin xác nhận là **nội dung** (bảng hợp nhất, bản nháp nguyên tắc), không phải bảng trạng thái quy trình — và phải là phản hồi thật của người dùng, cấm tự suy diễn "chắc đồng ý".
...
**Bảy vùng dưới đây là SÀN, không phải gợi ý.** Mỗi vùng buộc kết thúc bằng một trong ba kết cục, ghi tường minh ra bảng: (a) **≥1 luật kèm nguồn**, (b) dòng `đã kiểm — không có luật nào đang được cưỡng chế`, kèm **nói rõ đã đọc gì để kết luận thế**, hoặc (c) *(chỉ ở chế độ SỬA ĐỔI)* dòng `đã kiểm — luật đang cưỡng chế đã có trong hiến chương hiện hành` — kết cục (c) KHÔNG tính vào `A`, không đưa vào bảng hợp nhất. **CẤM im lặng bỏ vùng.** Bỏ vùng không ghi gì là bỏ sót, không phải "vùng không tồn tại" — và bạn sẽ không bao giờ biết mình đã bỏ sót, vì phép đếm `A` ở dưới đếm chính cái bạn tự chọn ghi.
...
```
số dòng hiển thị + số dòng đã gộp đi = A + B     (chế độ SỬA ĐỔI: = A + B + R)
```

- `số dòng đã gộp đi`: mỗi lần gộp k dòng thành 1 tính **k−1** (gộp 2 dòng = 1; gộp 3 dòng = 2).
- `R` *(chỉ chế độ SỬA ĐỔI)*: số nguyên tắc cũ được đưa vào bảng để gỡ/sửa — chúng là dòng hiển thị nhưng không thuộc `A` lẫn `B`, không cộng `R` thì phép cân đối không bao giờ cân.

Không cân bằng → bạn đã đánh rơi một dòng. Quay lại tìm, **không đi tiếp**. Rà một lần vẫn không cân → in **ánh xạ ID**: liệt kê từng `A#i`/`B#j` (và nguyên tắc cũ `R#k`) → `#dòng bảng`, mỗi ID xuất hiện đúng một lần (dòng riêng hoặc trong chú thích gộp) — ID không ánh xạ được chính là dòng rơi. Đây là điểm khác biệt so với việc chỉ "đếm cho có": một dòng làn B bị bỏ âm thầm sẽ làm vế trái hụt đi 1, và không có cách nào giấu.
...
**Khi cột `Code đã đạt?` không xác định được** — cấm để trống hay đoán bừa, xử theo hai nhánh:
- **Greenfield (đã bỏ làn A)**: phê `MUST` thẳng — `MUST + miễn trừ` chỉ tồn tại khi có code cũ vi phạm; KHÔNG chế ra vùng miễn trừ + "hạn chuẩn hóa" cho code chưa tồn tại.
- **Brownfield mà làn A không có dữ liệu cho dòng đó** (mong muốn ngoài 7 vùng quét), hoặc luật "đang cưỡng chế" nhưng đầy suppression/ngoại lệ hàng loạt → coi là **chưa đạt** → `MUST + miễn trừ`. Cấm phê `MUST` thẳng khi cột này là "chưa rõ" — đó chính là cơn bão CRITICAL từ `converge` mà mục dưới cảnh báo.
...
**Ép ngân sách**: sau khi quyết, **NHÓM** các dòng `MUST`/`MUST + miễn trừ` thành nguyên tắc dự kiến — thêm cột phụ `Nhóm nguyên tắc #` vào bảng, nhiều dòng cùng chủ đề nhóm vào một nguyên tắc nhiều bullet (đúng cách hiến chương spec-kit làm: 5 nguyên tắc, 42 `MUST` nằm trong các bullet). **NHÓM ≠ GỘP**: nhóm giữ nguyên từng dòng (mỗi dòng thành một bullet ở GĐ5), không đụng phép cân đối; "gộp" (xóa dòng) chỉ dành cho hai dòng trùng đúng một luật như định nghĩa hẹp ở trên. Đếm số **nhóm** — vượt **7** → nhóm lại hoặc hạ cấp (`→ Ràng buộc Kiến trúc` / `Bỏ`) cho tới khi ≤ 7.

Xin **xác nhận tường minh** bảng hợp nhất trước khi soạn nguyên tắc.

## Giai đoạn 5 — Soạn nguyên tắc + Cổng tự kiểm

**Chế độ SỬA ĐỔI**: chỉ soạn các nguyên tắc **mới hoặc bị sửa** đã chốt ở GĐ4. Nguyên tắc cũ không đổi → **giữ nguyên văn từng chữ**, không diễn đạt lại, không đánh số lại trừ khi có dòng bị gỡ. Cổng tự kiểm dưới đây chạy trên **toàn bộ** hiến chương sau khi ghép (cũ + mới), vì nguyên tắc cũ cũng có thể vi phạm Tầng 1. **Nguyên tắc CŨ trượt cổng** (Tầng 1 hoặc Tầng 2) → KHÔNG tự viết lại — "giữ nguyên văn" thắng "sửa ngay": gom mọi nguyên tắc cũ trượt vào một lượt AskUserQuestion (giữ nguyên / sửa theo đề xuất của bạn / gỡ), nêu rõ hệ quả semver của từng lựa chọn (sửa token chuẩn tắc = PATCH làm rõ; định nghĩa lại hoặc gỡ = MAJOR), người dùng chốt xong mới soạn. Luật "sửa ngay, lặp cho tới khi đạt" của Tầng 1 chỉ áp cho nguyên tắc **mới** do bạn vừa soạn.
...
6. **Falsifiable** — cột "Phép thử" phải viết được thành **một câu chỉ ra dữ liệu quan sát được** ("grep thấy secret trong source ⇒ trượt"; "endpoint không nằm trong whitelist mà thiếu decorator phân quyền ⇒ trượt"). **Không viết nổi phép thử một câu, hoặc phép thử chứa "tuỳ", "hợp lý", "phán đoán" ⇒ nguyên tắc đó TRƯỢT Tầng 2** — không có đường tự nhận đạt. Phép thử chỉ **lặp lại nguyên tắc ở thể phủ định** ("vi phạm X ⇒ trượt", "`converge` phát hiện vi phạm ⇒ trượt") cũng ⇒ **TRƯỢT** — phép thử hợp lệ phải nêu **artifact/thao tác cụ thể** (grep pattern gì, file/thư mục nào, config nào, lệnh nào) sao cho người chưa đọc nguyên tắc vẫn chạy được phép thử.
