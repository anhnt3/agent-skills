...
Cheatsheet là snapshot (version xác minh cuối ghi ở đầu file): nếu addon neo vào wording core
(preset wrap gần như chắc chắn có), **xác minh neo với đúng version spec-kit đích** — chạy
`scripts/check-core-anchors.sh` trên project đã init hoặc clone upstream; neo mất = finding
CRITICAL (gãy âm thầm), cheatsheet lỗi thời thì cập nhật luôn cheatsheet.
